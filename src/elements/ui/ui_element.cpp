...
namespace gui::element {
    ...
    void UiElement::window(SDL_Window *window) {
        this->_window = window;
    ...
    void UiElement::renderer(SDL_Renderer *renderer) {
    ...
    void UiElement::renderBorders() const {
        ...
        if (!SDL_GetRenderDrawColor(_renderer, &r, &g, &b, &a)) {
    ...
    void UiElement::renderVerticalScrollBar(int totalHeight, ui::Size clippedSize, int offset) const {
        ...
        float addedY =
            totalHeight == clippedHeight ? offset : offset / static_cast<float>((totalHeight - clippedHeight)) * (clippedHeight - scrollBarHeight);
